...
from ML_tools.classification.hbagging.hweak import HWeakClassifier
...
class HBagging():
    ...
    def fit(self, X_train, y_train, nb_classifiers=10, tolerance=0.1, list_features1=None, list_features2=None):
        ...
        if list_features1 is not None and list_features2 is not None:
            ...
            self.features_used = np.array(self.features_used[:self.nb_classifiers])
    ...
    def predict_proba(self, X_test):
        ...
        prediction_class_one = np.array(y_predict/self.nb_classifiers).reshape(-1,1)
    ...
    def predict(self, X_test, threshold=0.5):
        ...
        predictions = [1 if proba_predictions[i]>=threshold else 0 for i in range(len(proba_predictions))]
    ...
    def draw(self, X, y, features_names, reversed_variables_names=None, fig_path=None, title=None, name_classes=None, density=False, rename_dic={}, **kwargs): 
        ...
        if len(self.features_used)<4:
            fig, axes = plt.subplots(nrows=len(self.features_used), ncols=1, figsize=(24,16))
        elif len(self.features_used)<7:
            fig, axes = plt.subplots(nrows=len(self.features_used), ncols=1, figsize=(24,22))
        else:
            fig, axes = plt.subplots(nrows=len(self.features_used), ncols=1, figsize=(24,26))
...
#            distrib.fit(X_best_dim_sorted[class_ind_sorted==1])
            ...
            axis.hist(X_best_dim_sorted[class_inds_sorted==1], color="red", bins=bins, density=False, alpha=0.6, label=name_classes[1])
            axis.hist(X_best_dim_sorted[class_inds_sorted==0], color="mediumseagreen", bins=bins, density=False, alpha=0.6, label=name_classes[0])
...
#            
            ...
            axis.plot([threshold, threshold], [lims_y[0], lims_y[1]], color=color, linestyle="--", linewidth=4)
...
#            axis.plot([threshold, threshold], [-0.05, 0.05], color=color, linestyle="--")
            axis.text(s="Threshold : "+"%.2f"%threshold, x=threshold-0.05*range_x, y=lims_y[1]+0.2*range_y, fontsize=30, color=color)
...
#            
            ...
            axis.set_title(new_name, fontsize=40)#+", threshold: "+"%3.f"%threshold, fontsize=30)
    ...
    def draw_old(self, X, y, features_names, path_fig=None, title=None, **kwargs): 
        ...
        fig, axes = plt.subplots(nrows=self.nb_classifiers, ncols=1, figsize=(24,18))
        ...
        for i in range(self.nb_classifiers):
            ...
            axis.plot([threshold, threshold], [-0.05, 0.05], color=color, linestyle="--")
            axis.text(s="Threshold : "+"%.2f"%threshold, x=threshold-0.05, y=0.1, fontsize=14)
            ...
            axis.set_title(features_names[self.features_used[i]]+", threshold: "+str(threshold), fontsize=20)
...
#        X_test = X_test / std_X_train
        ...
        model.fit(X_train, y_train, nb_classifiers=best_nb_classifiers, tolerance=best_tolerance)      
        ...
        if plot:
            model.draw(X_train, y_train, title="training_"+str(k), features_names=variable_names, fig_path=fig_path)
            model.draw(X_test, y_test, title="testing_"+str(k), features_names=variable_names, fig_path=fig_path)
...
if __name__=="__main__":
    ...
    print("thresholds found", np.array(hbagging.thresholds)[np.array(hbagging.features_used)])
    ...
    hbagging.draw(X_train, y_train, title="training", features_names=features_names)
    hbagging.draw(X_test, y_test, title="testing", features_names=features_names)
