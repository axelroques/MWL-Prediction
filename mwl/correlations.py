...
features_cols = [col for col in table.columns if "feature" in col and "NASA_tlx" not in col]# and "ADF" not in col]# and "blinks_duration" not in col] #+ ["mean_theoretical_NASA_tlx"]  #+["flight_hours]
...
X_HBagging.iloc[:,np.array([list(X.columns).index("feature_tc_fixed_windows_"+feature) for feature in reverse_features if "feature_tc_fixed_windows_"+feature in list(X.columns)])] *= -1
